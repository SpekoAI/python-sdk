...
def _decode_sse_block(block: str) -> tuple[str, Any]:
    event = "message"
    data_lines: list[str] = []
...
def _iter_sse(chunks: Iterable[str]) -> Iterator[tuple[str, Any]]:
    buffer = ""
    for chunk in chunks:
        buffer += chunk
        while "\n\n" in buffer:
            ...
            if block.strip():
                yield _decode_sse_block(block)
    if buffer.strip():
        yield _decode_sse_block(buffer)


async def _aiter_sse(chunks: AsyncIterator[str]) -> AsyncIterator[tuple[str, Any]]:
    buffer = ""
...
def _sse_done_payload(events: Iterable[tuple[str, Any]]) -> dict[str, Any]:
    done: Optional[dict[str, Any]] = None
    for event, data in events:
        if event == "error":
            payload = data if isinstance(data, dict) else {}
            raise SpekoApiError(
                str(payload.get("error", data)),
                200,
                str(payload.get("code", "STREAM_ERROR")),
            )
        if event == "done":
            if not isinstance(data, dict):
                raise SpekoApiError("Malformed done event", 200, "STREAM_ERROR")
            done = data
    if done is None:
        raise SpekoApiError("Stream ended without a done event", 200, "STREAM_ENDED")
    return done


async def _async_sse_done_payload(
...
class Speko:
    ...
    def transcribe(
        ...
        *,
        ...
        with self._client.stream(
            ...
            _raise_for_status(resp)
            done = _sse_done_payload(_iter_sse(resp.iter_text()))
        return TranscribeResult.model_validate(done)
    ...
    def complete(
        ...
        *,
        ...
        with self._client.stream("POST", "/v1/complete", json=body) as resp:
            _raise_for_status(resp)
            done = _sse_done_payload(_iter_sse(resp.iter_text()))
        return CompleteResult.model_validate(done)
...
class AsyncSpeko:
    ...
    async def transcribe(
        ...
        *,
        ...
        async with self._client.stream(
            ...
            _raise_for_status(resp)
            done = await _async_sse_done_payload(_aiter_sse(resp.aiter_text()))
        return TranscribeResult.model_validate(done)
    ...
    async def complete(
        ...
        *,
        ...
        async with self._client.stream("POST", "/v1/complete", json=body) as resp:
            _raise_for_status(resp)
            done = await _async_sse_done_payload(_aiter_sse(resp.aiter_text()))
        return CompleteResult.model_validate(done)
